...
class ComCadAdapter(CadAdapter):
    ...
    def activate_layout(self, name: str) -> dict:
        """Make an existing layout active through the public COM API."""
        doc = self._doc()
        try:
            layout = doc.Layouts.Item(name)
        except Exception as exc:
            raise ValidationError(f"Layout not found: {name}") from exc
        try:
            doc.ActiveLayout = layout
        except Exception as exc:
            raise ValidationError(f"Failed to activate layout: {name}") from exc
        return {"active_layout": str(self._safe_get(self._safe_get(doc, "ActiveLayout"), "Name", name))}
